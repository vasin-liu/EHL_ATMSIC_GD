/**基本信息*/
var baseInfo;
/**设置基本信息*/
function setBaseInfo() {
	baseInfo = {
		jgid : $("cjgid").value,
		jgmc : $("cjgmc").value,
		pname : $("cpname").value,
		page : $("page").value,
		time : $("ctime").value
	};
}
/**页面信息*/
var pageInfo;
/**设置新增时的测试数据*/
function setTestValue(){
	$("pname").value = "李继忠";
	$("content").value = "就道路改革提几点意见";
	$("ajgid").value = "440100000000；";
	$("ajgmc").value = "广州交警支队；";
}
/**设置添加页面值*/
function setAddValue(){
	UDload.maxnum = 1;
	$("fileRegion").innerHTML = UDload.getForm(560);
	Tree.jgmc="ajgmc";
	Tree.jgid="ajgid";
	$("add").show();
//	setTestValue();
}

/**添加验证*/
function addCheck(){
	pageInfo = {};
	var pname = $("pname");
	var pnvalue = pname.value;
	if(pnvalue == ""){
		alert("请输入约稿人！");
		pname.focus();
		return false;
	}else{
		pageInfo.pname = pnvalue;
	}
	var content = $("content");
	var cvalue = content.value;
	if(cvalue == ""){
		alert("请输入约稿内容！");
		content.focus();
		return false;
	}else{
		pageInfo.content = cvalue;
	}
	var ajgid = $("ajgid");
	var ajvalue = ajgid.value;
	if(ajvalue == ""){
		alert("请选择接收单位！");
		$("aimg").focus();
		return false;
	}else{
		ajvalue = ajvalue.replace(/；/g,",");
		ajvalue = ajvalue.substring(0, ajvalue.length - 1);
		pageInfo.ajgids = ajvalue;
	}
	pageInfo.jgid = $("jgid").value;
	pageInfo.time = $("time").value;
	return true;
}

/**添加*/
function add(isValid){
	if (isValid == true || addCheck()) {
		if(!isValid){
			UDload.cbfname = "add(true)";
			UDload.upload();
		}else{
			if(UDload.result){
				pageInfo.apath = UDload.result;
			}
			var url = "policeWorks.orderContribution.add.d";
			new Ajax.Request(url, {
				method : "post",
				parameters : pageInfo,
				onComplete : function(xmlHttp) {
					var result = xmlHttp.responseText;
					if(result == "true"){
						alert("添加约稿信息成功！");
						window.location.reload();
					}else if(result == "false"){
						alert("添加约稿信息失败！");
					}else {
						alert("网络异常！");
					}
				}
			});
		}
	}
}

/**设置查询值*/
function setQueryValue(){
	var date = new Date();
	$("etime").value = date.format_(3);
	date.setDate(1);
	$("stime").value = date.format_(3);
}

/**查询页面对象字段名*/
var fnames = [ "id", "jgid", "jgmc", "pname", "time", "content","state" ];
/**查询页面对象字段描述*/
var fdescs = ["编号","机构编号","机构名称","约稿人","约稿时间","约稿内容","处理状态"];
/**处理状态信息*/
var sdescs = {"1":"未签收","2":"已签收"};
/**查询列表数据*/
var queryDatas;
/**查询单条数据*/
var queryData;
function query(){
	function siftTime(value,cname,refer){
		if(value && cname && refer){
			var format = "yyyy-MM-dd hh24:mi:ss";
			var time = " and to_date('" + value + "','"+format+"') ";
			time += refer;
			time += cname;
			return time;
		}
		return "";
	}
	var sql = "select oc.id,oc.jgid," +
	"(select jgmc from t_sys_department where jgid=oc.jgid)," +
	"oc.pname,to_char(oc.time,'yyyy-mm-dd hh24:mi'),content," +
	"(select state from t_oa_acceptdept where adid=oc.id and rpdcode='"+baseInfo.jgid+"')";
	sql += " from t_oa_ordercontribution oc";
	sql += " where 1=1";
	//约稿或者被约稿
	sql += " and (oc.jgid = '"+baseInfo.jgid+"' or instr(f_get_accdept(oc.id,2,1),'"+baseInfo.jgid+"')!=0)";
	//起始时间
	var start = $("stime").value;
	sql += siftTime(start+" 00:00:00","time","<=");
	//结束时间
	var end = $("etime").value;
	sql += siftTime(end+" 23:59:59","time",">=");
	sql += " order by oc.time desc";
	window.toPageNum = window.currPage;
	PageCtrl.initPage("tdData", "pageData", "pageNav", convertSql(sql),
			"showQueryResult", "10");
}

/**设置查询数据*/
function setQueryDatas(xmldoc) {
	if (fnames && xmldoc) {
		queryDatas = [];
		var rows = xmldoc.getElementsByTagName("row");
		var cols;
		for ( var i = 0; i < rows.length; i++) {
			cols = rows[i].childNodes;
			var queryData = {};
			for ( var j = 0; j < fnames.length && j < cols.length; j++) {
				queryData[fnames[j]] = cols[j].text == "null" ? ""
						: cols[j].text;
			}
			queryDatas.push(queryData);
		}
	} else {
		if (!fnames) {
			alert("未获取到查询列名！");
		}
		if (!xmldoc) {
			alert("xmldoc无效！");
		}
	}
}

/**显示查询结果*/
function showQueryResult(xmldoc){
	setQueryDatas(xmldoc);
	var str = getQueryTitle();
	str += "<table width='100%' border='0' cellpadding='0' cellspacing='0' class='table'>";
	str += "<tr class='titleTopBack'>";
	str += "<td width='5%' class='td_r_b td_font'>序号</td>";
	//要显示的字段名索引
	var tindexs = [ 2, 3, 4,  5, 6 ];
	//操作名称
	var onames = ["查看","处理","回稿"];
	//要显示的操作名称索引
	var onindexs = [ 0, 1 ];
	//进行总队设置
	if(baseInfo.jgid.substring(2,4)=="00"){
		tindexs = [ 2, 3, 4,  5 ];
		onindexs = [ 0, 1 ];
	}
	for ( var i = 0; i < tindexs.length; i++) {
		str += "<td class='td_r_b td_font'>" + fdescs[tindexs[i]] + "</td>";
	}
	for ( var i = 0; i < onindexs.length; i++) {
		str += "<td class='td_r_b td_font'>" + onames[onindexs[i]] + "</td>";
	}
	str += "</tr>";
	if (!queryDatas || queryDatas.length <= 0) {
		str += getQueryNull(tindexs.length + onindexs.length + 1);
	} else {
		var tds;
		var tdtext;
		var ibpath = "../../images/button/";
		//操作数据
		var odata;
		for ( var i = 0; i < queryDatas.length; i++) {
			queryData = queryDatas[i];
			tds = [];
			tds.push(getQueryTd(i+1));
			odata = [];
			odata.push(["para.gif","查看","openDetail("+i+",2)"]);
			if(queryData.jgid == baseInfo.jgid){
				odata.push(["update.gif","处理","openDetail("+i+",3)"]);
				odata.push(["lock.png","回稿",""]);
			}else{
				if(queryData.state == "1"){
					odata.push(["update.gif","处理","openDetail("+i+",3)"]);
				}else{
					odata.push(["lock.png","处理",""]);
				}
				odata.push(["update.gif","回稿","openReplayContribution("+i+",3)"]);
			}
			for ( var j = 0; j < tindexs.length; j++) {
				tdtext = queryData[fnames[tindexs[j]]];
				switch(fnames[tindexs[j]]){
					case "state":
						if(tdtext){
							tdtext = sdescs[tdtext];
						}else{
							tdtext = "..";
						}
					break;
					case "content":
						tdtext = getAttrByLimitLength(tdtext,20);
					break;
				}
				tds.push(getQueryTd(tdtext));
			}
			for ( var j = 0; j < onindexs.length; j++) {
				tdtext = "<img src='" + (ibpath + odata[onindexs[j]][0]) + "' alt='"+odata[onindexs[j]][1]+"' onclick='" + odata[onindexs[j]][2] + "' style='cursor:hand;' />";
				tds.push(getQueryTd(tdtext));
			}
			str += getQueryTr(tds.join(""));
		}
	}
	str += "</table>";
	var pagingObj = $('pageData').up('tr').next('tr').down('td');
	pagingObj.style.textAlign = "center";
	var tabOjb = document.getElementById("pageData");
	tabOjb.innerHTML = str;
	Popup.prototype.hideTips();
	if(window.toPageNum){
		onTurnToPage(toPageNum);
		toPageNum = null;
	}
}

function openDetail(index,page){
	if (index >= 0 && index < queryDatas.length) {
		var url = "orderContribution.jsp?page="+(page||2);
		var feature = "dialogWidth:800px;";
		var pwin = window.showModalDialog(url, queryDatas[index], feature);
		if(page == 3){
			query();
		}
	} else {
		alert("指定打开页面的索引无效！");
	}
}

function openReplayContribution(index){
	if(index >= 0 && index < queryDatas.length){
		var url = "newsFilesAdd.jsp";
		var feature = "dialogWidth:800px;dialogHeight:550px";
		var pwin = window.showModalDialog(url, queryDatas[index].id, feature);
		window.toPageNum = window.currPage;
		query();
	}else{
		alert("指定打开页面的索引无效！");
	}
}

function getDeptStates(ajginfos) {
	var stateHtml;
	if (ajginfos) {
		var colors = [ "red", "green", "black" ];
		var sep = "，";
		stateHtml = "";
		ajginfos = ajginfos.split(";");
		for ( var i = 0; i < ajginfos.length; i++) {
			if (ajginfos[i]) {
				stateHtml += sep + "<span style='color:" + colors[i] + ";'>"
						+ ajginfos[i] + "</span>";
			}
		}
		stateHtml = stateHtml.substring(sep.length);
	}
	return stateHtml;
}
/**约稿信息*/
var data;
/**设置约稿信息*/
function setData(){
	var url = "policeWorks.orderContribution.get.d";
	new Ajax.Request(url,{
		method:"post",
		asynchronous : false,
		parameters:{id:queryData.id},
		onComplete:function(xmlHttp){
			var xmlDoc = xmlHttp.responseXML;
			var orders = xmlDoc.getElementsByTagName("order");
			if (orders && orders.length == 1) {
				data = {};
				var oattrs = orders[0].attributes;
				for ( var i = 0; i < oattrs.length; i++) {
					data[oattrs[i].nodeName] = oattrs[i].nodeValue;
				}
			}
		}
	});
}

function setReadValue() {
	if(!data){return}
	//设置约稿机构、约稿人、约稿时间
	$("jgmc").value = data.jgmc;
	$("pname").value = data.pname;
	$("pname").disabled = true;
	$("time").value = data.time;
	//设置约稿内容
	$("contentTd").innerHTML = data.content;
	//设置附件值
	$("apathDescTd").innerText = "附件名称";
	$("fileRegion").innerHTML = UDload.getAttachHtmls(data.apath) || "暂无附件";
	//设置接收单位状态
	if (baseInfo.jgid == data.jgid) {
		$("rpdndescTd").innerText = "接收单位及处理状态";
		$("rpdnameTd").innerHTML = getDeptStates(data.ajginfos) || "暂无接收单位";
		$("rpdremarkTr").show();
	} else {
		$("rpdndescTd").innerText = "处理状态";
		$("rpdnameTd").innerHTML = sdescs[queryData.state];
	}
	//显示关闭按钮
	$("close").show();
}

function setDealValue(){
	if(!data){return}
	//设置约稿机构、约稿人、约稿时间
	$("jgmc").value = data.jgmc;
	$("pname").value = data.pname;
	$("pname").disabled = true;
	$("time").value = data.time;
	//设置约稿内容
	if(baseInfo.jgid == data.jgid){
		$("content").innerHTML = data.content;
	}else{
		$("contentTd").innerHTML = data.content;
	}
	//设置附件值
	$("apathDescTd").innerText = "附件名称";
	UDload.fwidth = 550;
//	UDload.fwidth = 450;
	UDload.maxnum = 1;
	$("fileRegion").innerHTML = UDload.getAttachHtmls(data.apath) || "暂无附件";
	//设置接收单位状态
	if (baseInfo.jgid == data.jgid) {
		$("rpdndescTd").innerText = "接收单位及处理状态";
		$("rpdnameTd").innerHTML = getDeptStates(data.ajginfos) || "暂无接收单位";
		$("rpdremarkTr").show();
	} else {
		$("rpdndescTd").innerText = "处理状态";
		$("rpdnameTd").innerHTML = sdescs[queryData.state];
	}
	//设置按钮
	if(baseInfo.jgid == data.jgid){
		$("modify").show();
	}else{
		if(queryData.state == 1){
			$("signIn").show();
		}
	}
	$("close").show();
}

function modifyCheck(){
	pageInfo = pageInfo || {};
	var content = $("content");
	var contentV = content.value;
	if(contentV == ""){
		alert("请输入约稿内容！");
		content.focus();
		return false;
	}else{
		if(contentV != data.content){
			pageInfo.content = contentV;
		}
	}
	var i = 0;
	for(var attr in pageInfo){
		i++;
	}
	if(i == 0){
		alert("您尚未修改任何信息！");
		content.focus();
		return false;
	}
	return true;
}

function modify(){
	if(modifyCheck()){
		pageInfo.id = data.id;
		var url = "policeWorks.orderContribution.modify.d";
		new Ajax.Request(url,{
			method:"post",
			parameters : pageInfo,
			onComplete:function(xmlHttp){
				var result = xmlHttp.responseText;
				if(result == "true"){
					alert("修改约稿信息成功！");
					window.close();
				}else if(result == "false"){
					alert("修改约稿信息失败！");
				}else {
					alert("网络异常！");
				}
			}
		});
	}
}

function signIn(){
	var params = {aid:data.id,state:"2",rpname:data.pname};
	Flow.updtState(params,"签收约稿信息",true);
}



window.onload = function(){
	setBaseInfo();
	if(!baseInfo.page){
		baseInfo.page = "0";
	}
	switch (baseInfo.page) {
	case "0":
		setAddValue();
		break;
	case "1":
		setQueryValue();
		query();
		break;
	case "2":
		queryData = window.dialogArguments;
		setData();
		setReadValue();
		break;
	case "3":
		queryData = window.dialogArguments;
		setData();
		setDealValue();
		break;
	}
}




function getQueryTd(text, width) {
	if (width) {
		width = "width='" + width + "%'";
	} else {
		width = "";
	}
	var td = "<td " + width + "  class='td_r_b td_font' align='center'>";
	td += text;
	td += "</td>";
	return td;
}

function getQueryTr(td, height) {
	return "<tr align='center' height='" + (height || 25) + "'>" + td + "</tr>";
}

function getQueryTitle() {
	var title = "<span class='currentLocationBold'>查询结果</span>";
	title = "<div style='text-align:center;width:100%;line-height:22px; float:left;'>"
			+ title + "</div>";
	return title;
}

function getQuerySubTitle(tindexs,scndescs) {
	if (tindexs && scndescs) {
		var tds = getQueryTd("序号");
		for ( var i = 0; i < tindexs.length; i++) {
			tds += getQueryTd(scndescs[tindexs[i]]);
		}
		return getQueryTr(tds);
	}
	return null;
}

function getValueAttr(value, width) {
	var style = "width:" + width + "px;";
	style += "overflow: hidden;white-space: nowrap;text-overflow:ellipsis;";
	return "<span title='" + value + "' style='" + style + "'>" + value + "</span>";
}

function getAttrByLimitLength(value, length) {
	var attrValue = value;
	if (value.length > length) {
		var attrMark = "..";
		attrValue = value.substring(0, length - attrMark.length) + attrMark;
	}
	return "<span title='" + value + "'>" + attrValue + "</span>";
}

function getQueryNull(length) {
	var queryNull = "<td class='td_r_b td_font' colspan='" + (length || 0)
			+ "' align='center'>此条件下没有数据</td>";
	queryNull = "<tr class='titleTopBack'>" + queryNull + "</tr>";
	return queryNull;
}