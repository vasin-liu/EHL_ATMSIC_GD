...
CalendarDT.prototype.draw = function() {
   ...
   //时间输入
   mvAry[mvAry.length]  = '        <td colspan="5" 	width="71.4%" style="fontsize:12px;font-weight: normal;">'+CalendarDT.language["timetip"][this.lang]+'<select name="calendarHourDT" id="calendarHourDT" style="font-size:12px;"></select>:<select name="calendarMinDT" id="calendarMinDT" style="font-size:12px;"></select></td>';
  
   //关闭按钮
   mvAry[mvAry.length]  = '        <td colspan="2"><input name="calendarCloseDT" type="button" id="calendarCloseDT" value="' + CalendarDT.language["close"][this.lang] + '" style="border: 1px solid ' + calendarDT.colors["input_border"] + ';background-color:' + calendarDT.colors["input_bg"] + ';width:100%;height:20px;font-size:12px;"/></td>';
   mvAry[mvAry.length]  = '      </tr>';
   mvAry[mvAry.length]  = '    </table>';
