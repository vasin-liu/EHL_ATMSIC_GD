...
var btnIDList = ["addAccident","addCongestion","addRoadBuild","addTrafficRestrain"];
...
// 	this.affairSRCTRIONECode = '002001';	//三台合一事件来源编码
	...
	for(var i=0; i<this.m_watchingData['affairData'].length; i+=1){
		...
		if(dataItem.eventState_code  == this.handleCompleteCode){	
...
//未处理警情表格定义DispatchWatching.prototype.affairDataTable = function(affairData){			
	...
	this.affairList.setColumnType(["CHECKBOX","TEXT","TEXT","TEXT","TEXT"]);
	this.affairList.setTableHead( ["选择","报警时间","事件来源","事件类型","事件状态"] );  //"报警单号",
	this.affairList.setCellWidth(['4%','15%','22%','20%','29%']);
