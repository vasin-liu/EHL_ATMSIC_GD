...
var strFeedType;
var EVENTSTATE = "";
function modifyGeoLogicDisaster(feedType){
...
//	alert("test-->:"+strFeedType);
	...
	var cjr = $("cjr");
	var cjsj = $("cjsj");
	var djsj = $("geologicdjsj");
	var jdhb = $("jdhb");
	  
//	var ALARMID = $("ALARMID");
	var ajfssj = $("ajfssj");   
	var ajjssj = $("geologicajjssj");   
	var ajlx = $("ajlx_feedBack");     
	var yxdl_GeoLogicDisaster = $("yxdl_GeoLogicDisaster");   
	var bjfw_GeoLogicDisaster = $("bjfw_GeoLogicDisaster");
...
//	var fkzj = $("fkzj");
	...
	params["yxdl"] = yxdl_GeoLogicDisaster.value;
	params["bjfw"] = bjfw_GeoLogicDisaster.value;
...
//		alert(eventType);
	...
	url = encodeURI(url);
	new Ajax.Request(url, {method:"post", parameters:params, onComplete:modifyGeoLogicDisasterResponse});
	
}
//完成后返回处理<按钮的隐藏>
function modifyGeoLogicDisasterResponse(xmlHttp){
...
//	alert("xmlDoc:"+xmlDoc);
	if(xmlDoc == "T"){
		...
		switch (strFeedType) {
			case 'cj':
				$("cjbtn_GeoLogicDisaster").style.display = "none";
				$("djsj").value = getSysdate(0);
				break;
			case 'ddxc':
				$("cjbtn_GeoLogicDisaster").style.display = "none";
				$("ddxcbtn_GeoLogicDisaster").style.display = "none";
				if($("ajjssj").value != ""){
			...
			case 'clwb':
				...
				$("cjbtn_GeoLogicDisaster").style.display = "none";
				$("ddxcbtn_GeoLogicDisaster").style.display = "none";
				$("clwbbtn_GeoLogicDisaster").style.display = "none";
				break;
			default:
				break;
		}
