...
var strFeedType;
var EVENTSTATE = "";
function modifyPoliceEvent(feedType){
...
//	var ALARMID = $("ALARMID");
	var ajfssj = $("ajfssj");   
	var ajjssj = $("policeajjssj");   
	var ajlx = $("ajlx_feedBack");     
	var yxdl_PoliceEvent = $("yxdl_PoliceEvent");   
	var bjfw_PoliceEvent = $("bjfw_PoliceEvent");
...
//	params["alarmState_Exception"] = alarmState_Exception.value;
	...
	params["yxdl"] = yxdl_PoliceEvent.value;
	params["bjfw"] = bjfw_PoliceEvent.value;
...
//		alert(eventType);
	...
	url = encodeURI(url);
	new Ajax.Request(url, {method:"post", parameters:params, onComplete:modifyPoliceEventResponse});
	
}
//完成后返回处理<按钮的隐藏>
function modifyPoliceEventResponse(xmlHttp){
...
//	alert("xmlDoc:"+xmlDoc);
	if(xmlDoc == "T"){
		...
		switch (strFeedType) {
			case 'cj':
				$("cjbtn_PoliceEvent").style.display = "none";
				$("djsj").value = getSysdate(0);
				initFeedBackPage();
				break;
			case 'ddxc':
				$("cjbtn_PoliceEvent").style.display = "none";
				$("ddxcbtn_PoliceEvent").style.display = "none";
				if($("ajjssj").value != ""){
			...
			case 'clwb':
				...
				$("cjbtn_PoliceEvent").style.display = "none";
				$("ddxcbtn_PoliceEvent").style.display = "none";
				$("clwbbtn_PoliceEvent").style.display = "none";
