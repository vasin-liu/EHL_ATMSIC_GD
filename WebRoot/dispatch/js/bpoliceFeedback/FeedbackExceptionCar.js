...
var strFeedType;
var EVENTSTATE = "";
function modifyExceptionCar(feedType){
...
//	var cjdw = $("cjdw");
	var cjr = $("cjr");
	var cjsj = $("cjsj");
	//var djsj = $("exceptioncardjsj");
	var djsj = $("exceptioncardjsj");
	//alert("time"+djsj.value);
	var jdhb = $("jdhb");
//	var ALARMID = $("ALARMID");
	var ajfssj = $("ajfssj");   
	var ajjssj = $("exceptioncarajjssj");   
	var ajlx = $("ajlx_feedBack");     
	var sjxm_Exception = $("sjxm_Exception");   
	var gzcdw_Exception = $("gzcdw_Exception");
...
//	params["alarmState_Exception"] = alarmState_Exception.value;
	...
	params["sjxm"] = sjxm_Exception.value;
	params["gzcdw"] = gzcdw_Exception.value;
...
//		alert(eventType);
	...
	url = encodeURI(url);
	new Ajax.Request(url, {method:"post", parameters:params, onComplete:modifyExceptionCarResponse});
	
}
//完成后返回处理<按钮的隐藏>
function modifyExceptionCarResponse(xmlHttp){
	...
	if(xmlDoc == "T"){
		...
		switch (strFeedType) {
			case 'cj':
				$("cjbtn_ExceptionCar").style.display = "none";
				$("djsj").value = getSysdate(0);
				break;
			case 'ddxc':
				$("cjbtn_ExceptionCar").style.display = "none";
				$("ddxcbtn_ExceptionCar").style.display = "none";
				if($("ajjssj").value != ""){
			...
			case 'clwb':
				...
				$("cjbtn_ExceptionCar").style.display = "none";
				$("ddxcbtn_ExceptionCar").style.display = "none";
				$("clwbbtn_ExceptionCar").style.display = "none";
