...
dhtmlXTreeObject.prototype._attachChildNode = function (parentObject, itemId, itemText, itemActionHandler, image1, image2, image3, optionStr, childs, beforeNode, afterNode) {
	...
	if (image1) {
		Nodes[Count].imgs[0] = image1;
	}
	if (image2) {
		Nodes[Count].imgs[1] = image2;
	}
	if (image3) {
		Nodes[Count].imgs[2] = image3;
	}
...
dhtmlXTreeObject.prototype._correctPlus = function (itemObject) {
	...
	if ((this.XMLsource) && (!itemObject.XMLload)) {
		...
		this._setSrc(imsrc2, this.imPath + itemObject.imgs[2]);
		if (this._txtimg) {
		...
		if ((itemObject.childsCount) || (itemObject.unParsed)) {
			if ((itemObject.htmlNode.childNodes[0].childNodes[1]) && (itemObject.htmlNode.childNodes[0].childNodes[1].style.display != "none")) {
				...
				this._setSrc(imsrc2, this.imPath + itemObject.imgs[1]);
				if (this._txtimg) {
					return (imsrc.innerHTML = "[-]");
				}
			} else {
			...
			this._setSrc(imsrc2, this.imPath + itemObject.imgs[0]);
		}
	}
	var tempNum = 2;
...
dhtmlXTreeObject.prototype.openAllItems = function (itemId) {
    if (itemId==window.undefined) itemId=this.rootId; //Linbh 加入.参数为空时默认展开根节点
...
dhtmlXTreeObject.prototype.doCut = function () {
	...
	for (var i = 0; i < this.nodeCut.length; i++) {
		...
		tempa._cimgs[0] = tempa.imgs[0];
		tempa._cimgs[1] = tempa.imgs[1];
		tempa._cimgs[2] = tempa.imgs[2];
		tempa.imgs[0] = tempa.imgs[1] = tempa.imgs[2] = this.cutImage;
...
dhtmlXTreeObject.prototype.clearCut = function () {
	for (var i = 0; i < this.nodeCut.length; i++) {
		var tempa = this.nodeCut[i];
		tempa.imgs[0] = tempa._cimgs[0];
		tempa.imgs[1] = tempa._cimgs[1];
		tempa.imgs[2] = tempa._cimgs[2];
		this._correctPlus(tempa);
	}
	this.nodeCut = new Array();
};
...
dhtmlXTreeObject.prototype._recreateBranch = function (itemObject, targetObject, beforeNode, level) {
	...
	var newNode = this._attachChildNode(targetObject, itemObject.id, itemObject.label, 0, itemObject.imgs[0], itemObject.imgs[1], itemObject.imgs[2], st, 0, beforeNode);
