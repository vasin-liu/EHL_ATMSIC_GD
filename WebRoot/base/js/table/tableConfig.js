...
function onLoadTableGrid() {
	...
	tablegrid.init();
	tablegrid.loadXML("base.datatable.getData.d");
...
function onLoadFieldGrid()
{
	...
	fieldgrid.init();
	fieldgrid.loadXML("base.datafield.getData.d");
...
function onLoadFieldGridByTid(){
	var url= "base.datafield.getDataByTableId.d";
	url=encodeURI(url);
...
function doFieldDelete() {
  if(getFieldId()!=""){
	if (confirm("您确定删除此项记录吗?")) {
		var getparam = "";
		var url = "base.datafield.delete.d?getparam=" + getFieldId();
		url = encodeURI(url);
...
function doTableDelete() {
  if(getTableId()!=""){
	if (confirm("您确定删除此项记录吗?")) {
		var url = "base.datatable.delete.d?getparam=" + getTableId();
		url = encodeURI(url);
...
function FilterByTable() {
    ...
    var url = "base.datatable.filter.d";
	url = encodeURI(url);
...
function FilterByField(){
	...
	var url="base.datafield.filter.d";
	url=encodeURI(url);
