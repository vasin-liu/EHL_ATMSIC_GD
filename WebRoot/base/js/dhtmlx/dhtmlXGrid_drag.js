...
dragContext.prototype.ind = function () {
	...
	return (ind + 1 + ((this.target == "treeGrid" && this.tobj._h2.get[this.tobj.rowsCol[ind].idd].status == "minus") ? this.tobj._getOpenLenght(this.tobj.rowsCol[ind].idd, 0) : 0));
};
dragContext.prototype.img = function () {
	if ((this.target != "treeGrid") && (this.sobj._h2)) {
...
dhtmlXGridObject.prototype._drag = function (sourceHtmlObject, dhtmlObject, targetHtmlObject, lastLanding) {
	...
	if (((c.tobj.dadmode == 2) && (c.tobj.dadmodec == 1)) && (c.tobj.dadmodefix < 0)) {
		if (c.tobj.obj._rows(0).idd != c.tid) {
			c.tid = r1.previousSibling.idd;
		} else {
			c.tid = window.unknown;
		}
	}
	...
	if ((c.tobj.dragFunc) && (!c.tobj.dragFunc(c.slist(), c.tid, c.sobj, c.tobj, c.sExtra, c.tExtra))) {
		return;
	}
	...
	if (c.tobj.dropFunc) {
		c.tobj.dropFunc(result.join(","), c.tid, result.join(","), c.sobj, c.tobj, c.sExtra, c.tExtra);
	}
	...
	if (this._h2) {
		...
		if (akids.childs.length) {
			for (var i = 0; i < akids.childs.length; i++) {
				this._nonTrivialNode(tree, newone, 0, this.rowAr[akids.childs[i].id], 1);
				if (!this.dpcpy) {
					i--;
				}
			}
