					...
					b.jgrid,
					{
						...
						stringToDoc : function(d) {
							...
							if (typeof d !== "string")
								return d;
							try {
								i = (new DOMParser).parseFromString(d,
										"text/xml")
							} catch (g) {
							...
							return i
									&& i.documentElement
									&& i.documentElement.tagName != "parsererror" ? i
									: null
						},
						parse : function(d) {
							...
							d || (d = "{}");
							return b.jgrid.useJSON === true
									&& typeof JSON === "object"
									&& typeof JSON.parse === "function" ? JSON
									.parse(d) : eval("(" + d + ")")
						},
						parseDate : function(d, i) {
							...
							if (i && i !== null && i !== undefined) {
								...
								i = i.split(c);
								d = d.split(c);
								var l = b.jgrid.formatter.date.monthNames, a = b.jgrid.formatter.date.AmPm, q = function(
										x, y) {
									if (x === 0) {
										if (y === 12)
											y = 0
						...
						if (this.tagName.toUpperCase() != "TABLE")
							...
							if (this.p.colNames.length !== this.p.colModel.length)
								...
								var Ea = function(e, f, j, m) {
									if (a.p.colModel[f].sortable)
										if (!(a.p.savedRow.length > 0)) {
											...
											b("span.ui-grid-ico-sort", j)
													.addClass(
															"ui-state-disabled");
											b(j).attr("aria-selected", "false");
											b("span.ui-icon-" + a.p.sortorder,
													m).removeClass(
													"ui-state-disabled");
											...
											e = e
													.substring(5 + a.p.id.length + 1);
											a.p.sortname = a.p.colModel[f].index
													|| e;
											j = a.p.sortorder;
											if (b.isFunction(a.p.onSortCol))
												if (a.p.onSortCol.call(a, e, f,
														j) == "stop") {
													a.p.lastsort = f;
													return
												}
											if (a.p.datatype == "local")
												a.p.deselectAfterSort
														&& b(a)
																.jqGrid(
																		"resetSelection");
											else {
								...
								if (this.p.multiselect) {
									...
									b("#cb_" + b.jgrid.jqID(a.p.id), this)
											.bind(
													...
													function() {
														a.p.selarrrow = [];
														var e = a.p.frozenColumns === true ? a.p.id
																+ "_frozen"
																: "";
														if (this.checked) {
															b(a.rows)
										...
										.click(
												function(e) {
													...
													var f = b(Y).hasClass(
															"cbox"), j = true;
													if (b
															.isFunction(a.p.beforeSelectRow))
														j = a.p.beforeSelectRow
																.call(
																		a,
																		Q[0].id,
																		e);
													if (Y.tagName == "A"
															|| (Y.tagName == "INPUT"
																	|| Y.tagName == "TEXTAREA"
																	|| Y.tagName == "OPTION" || Y.tagName == "SELECT")
															&& !f)
														return this;
													if (j === true) {
														if (a.p.cellEdit === true)
															...
															if (a.p.multiselect
																	&& a.p.multiboxonly)
																if (!f) {
																	var p = a.p.frozenColumns ? a.p.id
																			+ "_frozen"
																			: "";
																	b(
																			a.p.selarrrow)
												...
												function(e, f) {
													...
													if (f && f.page) {
														...
														if (j < 1)
															j = 1;
														a.p.page = j;
														a.grid.bDiv.scrollTop = a.grid.prevRowHeight ? (j - 1)
																* a.grid.prevRowHeight
																* a.p.rowNum
																: 0
													}
													if (a.grid.prevRowHeight
															&& a.p.scroll) {
														delete a.p.lastpage;
														a.grid
																.populateVisible()
										...
										.append(
												b(
														'<div style="position:relative;'
																+ (y
																		&& b.browser.version < 8 ? "height:0.01%;"
																		: "")
																+ '"></div>')
														.append("<div></div>")
														.append(this))
								...
								if (y) {
									...
									a.p.multikey
											&& b(h.bDiv).bind("selectstart",
													function() {
														return false
													})
								} else
									a.p.multikey
											&& b(h.bDiv).bind("mousedown",
													function() {
														return false
													});
								ea && b(h.bDiv).hide();
								...
								if (a.p.toolbar[0]) {
									...
									if (a.p.toolbar[1] == "both") {
										...
										b(h.uDiv).width(h.width).addClass(
												"ui-userdata ui-state-default")
												.attr("id", "t_" + this.id);
									ea && b(h.uDiv).hide()
								}
								if (a.p.toppager) {
									a.p.toppager = b.jgrid.jqID(a.p.id)
											+ "_toppager";
									h.topDiv = b("<div id='" + a.p.toppager
											+ "'></div>")[0];
								...
								if (d !== undefined) {
									...
									if (!(!k || !k.className || k.className
										...
										if (c.p.multiselect) {
											...
											c.p.selrow = k.id;
											l = b.inArray(c.p.selrow,
													c.p.selarrrow);
											if (l === -1) {
												k.className !== "ui-subgrid"
														&& b(k)
																.addClass(
																		"ui-state-highlight")
																.attr(
												...
												b("#jqg_"
														...
														+ "_"
														+ b.jgrid
																.jqID(c.p.selrow))[c.p.useProp ? "prop"
														: "attr"]("checked", h);
											...
											if (q) {
												...
												b(
														"#jqg_"
																...
																+ b.jgrid
																		.jqID(d),
														"#" + b.jgrid.jqID(q))[c.p.useProp ? "prop"
														: "attr"]("checked", h)
											}
											c.p.onSelectRow
													&& i
													&& c.p.onSelectRow.call(c,
															k.id, h)
...
(function(b) {
	...
	b.unformat = function(a, c, d, f) {
		var g, e = c.colModel.formatter, h = c.colModel.formatoptions || {}, i = /([\.\*\_\'\(\)\{\}\+\?\\])/g, j = c.colModel.unformat
				|| b.fn.fmatter[e] && b.fn.fmatter[e].unformat;
		if (typeof j !== "undefined" && b.isFunction(j))
			...
			case "currency":
				...
				c = h.thousandsSeparator.replace(i, "\\$1");
				g = b(a).text().replace(RegExp(c, "g"), "").replace(
						h.decimalSeparator, ".").replace(h.prefix, "").replace(
						h.suffix, "");
				break;
			case "checkbox":
				h = c.colModel.editoptions ? c.colModel.editoptions.value
						.split(":") : [ "Yes", "No" ];
		...
		return g !== undefined ? g : f === true ? b(a).text() : b.jgrid
				.htmlDecode(b(a).html())
	};
													...
													if (h) {
														...
														var r = c.jgrid
																.createEl(
																		...
																		"",
																		true,
																		a.p.ajaxSelectOptions,
																		true);
														c(r).addClass(
																"input-elm");
														i = h.searchoptions.sopt ? h.searchoptions.sopt
																: a.p.sopt ? a.p.sopt
																		: h.searchtype === "string" ? a.p.stropts
																				: a.p.numopts;
														var u = "", A = 0;
														t = [];
														c
									...
									if (!(h == "subgrid" || h == "cb" || h == "rn")) {
										...
										if (i.editable === true
											...
											if (b.isFunction(c.p.formatCell)) {
												var j = c.p.formatCell.call(c,
														c.rows[d].id, h, e, d,
														f);
												if (j !== undefined)
													e = j
											}
											j = b.extend({}, i.editoptions
													|| {}, {
												id : d + "_" + h,
												name : h
											});
											...
											b.isFunction(c.p.beforeEditCell)
													&& c.p.beforeEditCell.call(
															c, c.rows[d].id, h,
															e, d, f);
											b(g).html("").append(m).attr(
													"tabindex", "0");
											window.setTimeout(function() {
												b(m).focus()
											}, 0);
											...
											g
													.addClass("edit-cell ui-state-highlight");
											b(c.rows[d])
													.addClass(
															"selected-row ui-state-hover");
							...
							.each(function() {
								...
								if (!(!this.grid || this.p.cellEdit !== true)) {
									...
									if (a !== false)
										...
										this.p.savedRow.length > 0
												&& b(this).jqGrid("saveCell",
														d, f)
								}
							})
				},
