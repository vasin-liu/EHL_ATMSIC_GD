...
<%
  ...
  //  String jgmc=strObj[1];//单位名称
	...
	String DeviceIP = StringHelper.obj2str(request.getParameter("DeviceIP"),"");
	String DevicePort = StringHelper.obj2str(request.getParameter("DevicePort"),"");
	String DeviceUsername = StringHelper.obj2str(request.getParameter("DeviceUsername"),"");
	String DevicePasswd = StringHelper.obj2str(request.getParameter("DevicePasswd"),"");
	int ChannelList = StringHelper.obj2int(request.getParameter("ChannelList"),0);
	String cmpc = StringHelper.obj2str(request.getParameter("cpmc"),"");
	ChannelList -= 1;
%>
<html>
	<head>
		...
		<style type="text/css">
			...
			#BodyRight {
				width: 100%;
				height:height:expression((body.clientWidth*3)/4);
				position:absolute;
				left:0;
				top:0;
				float: left;
			...
			#OCXBody {
				width:100%;
				height:expression((body.clientWidth*3)/4);
			}
			#OperatLogBody {
				width:100%;
				height:150px;
		...
		<script type="text/javascript">
			window.onunload=function(){
				//if(event.clientX<0&&event.clientY<0){   //2011-07-30 zhoucr 屏蔽
					try{
			...
			*************************************************/
			window.onload = function(){ 
				...
				//5分钟后自动关闭视频窗口
				window.setTimeout("window.close();", 5*60*1000);
			} 

			CurrentChl = 0 ;
			function ShowVideo(){
				document.getElementById("DeviceIP").value=<%=DeviceIP%>; 
				document.getElementById("DevicePort").value=<%=DevicePort%>; 
				document.getElementById("DeviceUsername").value=<%=DeviceUsername%>; 
				document.getElementById("DevicePasswd").value=<%=DevicePasswd%>; 
				ButtonPress("LoginDev");
				
				document.getElementById("ChannelList").value=<%=ChannelList%>;//第一通道
				CurrentChl = <%=ChannelList%>;
				ButtonPress("Preview:start");
			}
			var privateCode='<%=jgccbm%>';
	...
	try{
		switch (sKey){
			...
			case "Preview:start":{
				...
				m_iNowChanNo = parseInt(CurrentChl);
				if(m_iNowChanNo > -1){
					if(m_iPlay == 1){
    ...
    <table width="100%" cellspacing="1" cellpadding="0" border="0" bgcolor="#96C3E4">
      ...
      <tr bgcolor="#f5f5f5" height="30">
        <td class="distanceleft"> IP:
          <input type="text" id="DeviceIP" value="" class="normalinput">
          port:
    ...
    <div id="BodyRight" align="center">
      <div id="OCXBody">
        <div class="smallocxdiv" id="NetPlayOCX1" >
          <object classid="CLSID:CAFCF48D-8E34-4490-8154-026191D73924" codebase="../../activex/NetVideoActiveX23.cab#version=2,3,13,1" standby="Waiting..." id="HIKOBJECT1" width="100%" height="100%" name="HIKOBJECT1" ></object>
        </div>
        <div class="smallocxdiv" id="NetPlayOCX2" style="display:none;">
        ...
        <div class="smallocxdiv" id="NetPlayOCX3" style="display:none;">
        ...
        <div class="smallocxdiv" id="NetPlayOCX4" style="display:none;">
      ...
      <div id="OperatLogTitle" >日志（运行结果）<span style="margin-left:170px; cursor:pointer" onClick="ArrangeWindow(1)">一画面</span>
      	<span style="margin-left:20px; cursor:pointer" onClick="ArrangeWindow(4)">四画面</span>
      </div>
      <div id="OperatLogBody" ></div>
    </div>
  </div>
