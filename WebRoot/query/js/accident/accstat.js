...
{	
	...
	doClick : function(){
		...
		var stattype = "";//统计方式标志 1：事故地点 2：车辆类型  3：事故类型 
		for(var i = 0; i<radios.length ; i++){
			if(radios[i].checked == true){
		...
		switch(parseInt(timetype)){
		    ...
		    case 4:colsNum = "12 ";break;
		    default:colsNum = "31";break;
		}
		//定义并赋值公用变量（统计类型-statTypeDesc）
		switch(parseInt(stattype)){
		    case 1:statTypeDesc = "事故地点";break;
		    case 2:statTypeDesc = "车辆类型";break;
		    case 3:statTypeDesc = "事故类型 ";break;
		    default:statTypeDesc = "";break;
		}
		
		var url = "tira.AccStatAction.getData.d";
