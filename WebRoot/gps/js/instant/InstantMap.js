 ...
 * 获取指定范围内的GPS信息
 */
InstantMap.prototype.readGPSPoints = function()
{
 ...
 * 定时 刷新 GPS 位置信息
 */
InstantMap.prototype.refreshPoints = function()
{
    gpsmap.readGPSPoints();
    if(!gpsInterId)
    {
   		gpsInterId = setInterval("gpsmap.readGPSPoints()", gpsInterval * 1000);
   	}
};
 ...
 */
InstantMap.prototype.finishReadPoints = function(xmlHttp)
{
 ...
 * 解析单条GPS数据
 */
InstantMap.prototype.getCarInfo = function( rowXML )
{
 ...
 * 在地图上显示GPS车辆
 */
InstantMap.prototype.showAllCarPoint =  function()
{
 ...
 * 判断是否在缓冲区域范围内
 */
InstantMap.prototype.isBoundInBuffer = function()
{
 ...
 * 更新缓冲区域，获取新区域上GPS数据
 */
InstantMap.prototype.readBoundPoints = function()
{
 ...
 * 获取地图上当前显示区域范围
 */
InstantMap.prototype.readCurrentBound = function()
{
	var thisZoom = map.getCurrentZoom();
 ...
 * 构造GPS标注 图标 元素
 */
InstantMap.prototype.creatCarImgElement = function(carCode,carType,isOnline)
{
 ...
 * 在地图上绘制GPS点元素
 */
InstantMap.prototype.paintCarPoint = function(symbolID, screenCoord ,carData)
{
...
/**
  ...
  */
InstantMap.prototype.showCarSymbol = function(mapDiv, infoCoord, zoom, order, currCar)
{
 	//由经纬度转屏幕坐标.
 ...
 * 判断停止限制，展示不同样式 
 */
InstantMap.prototype.checkStopLimit = function(stoptime,oCarEle)
{	
 ...
 * 判断越辖区限制，展示不同样式 
 */
InstantMap.prototype.checkOutOfRegion = function( outInfo ,oCarEle )
{
 ...
 * 获取GPS数据展示html字符串
 */
InstantMap.prototype.getCarSummery = function( carData )
{
	var strHtml = '<table>';
 ...
 * 显示信息窗体
*/
InstantMap.prototype.showMapTips = function(event, carData)
{		
    var x = 0;
    var y = 0;
    ...
    {	    	
		...
		carData = gpsmap.m_currentData[this.dataIndex] ;
    }	
    else
    {
	...
	//显示正在查询提示		
	var strHtml = gpsmap.getCarSummery(carData);
 ...
 * 
 */
InstantMap.prototype.centerCarInfo = function(carCode)
{
 ...
 * 获取指定GPS的信息
 */
InstantMap.prototype.readInfoByCarCode = function(carCode)
{
 ...
 * 解析单个GPS的数据
 */
InstantMap.prototype.finishGetCarInfo = function(xmlHttp)
{
	var xmlDoc = xmlHttp.responseXML;
 	var results = xmlDoc.getElementsByTagName("row");
  	var carData =  null;
 ...
 * GPS选择事件处理
 */
InstantMap.prototype.carChoiceHandler = function(carData)
{
	//显示选中的禁行线路详细信息				
 ...
 * 设置选中GPS相关数据
 */ 
InstantMap.prototype.setGpsCarInfo = function(carcode,index)
{		
	//控制选中状态
 ...
 */
InstantMap.prototype.getGpscarQuery = function(queryContainer)
{
	//形成查询条件
...
//	var txtClassifyQ = $('txtClassifyQ').value;
	//var queryCond = " ";
	//update by jason 20091229修改原因：为了拼查询条件后面的“and”
	var queryCond = " 1=1 ";
	...
	//PageCtrl.initPage("gpscarQueryResult","pageData","pageNav",selectSQL,"showFilterResponse","10");
	//update by Jason 20091225 修改原因：采用多数据源
	//prompt("",selectSQL);
	PageCtrl.initPage("gpscarQueryResult","pageData","pageNav",selectSQL,"showFilterResponse","10","gps");
...
function showFilterResponse(xmlDoc){
	...
	var str = "<table id=\"tabGPSCarList\" class=\"scrollTable\" width=100% cellSpacing=0 cellPadding=0 border=0>";
	
	//生成表头
	var heads  = ['车牌号码','呼叫号码','在 线','所属辖区'];
	var colWidths =[70,70,40,90];
	 
	str += "<tr class=\"scrollColThead\"><th class=\"title\" align=center width=15%>选择</th>";
	for(var i = 0 ; i < heads.length; i+=1)
	{
		var subHead = heads[i];
		str += "<th align=\"center\" class=\"title\" width=" + colWidths[i] + ">";
		str += subHead;
		str += "</th>";
	}											
	str += "</tr>";
	...
	{
		var rowResult = rows[i].childNodes;
		...
		str += "<tr class=\"rowstyle\" nowrap onclick=\"gpsmap.setGpsCarInfo('"+  carCode + "','"+ (i+1) +"',"+ coord +");\">";	
		str += "<td align=\"center\">";
		str += "<input type=\"checkbox\" id=\"chk_sel_"+i+"\"  /> </td>";
		for(var j=0;j<showCols.length;j+=1)
		{
		    if(showCols[j]=='null') {
		       showCols[j] = '&nbsp;';
		    }
		    str += "<td>" + showCols[j] + "</td>";
		}					    					    
		str += "</tr>";						
	}
    str += "</table>";
 ...
 * 展现查询得到GPS数据列表
 */ 
InstantMap.prototype.responseQuery = function(xmlHttp)
{
 	var xmlDoc = xmlHttp.responseXML;
	...
	for (var i = 0; i < results.length; i+=1)
	{
		...
		//class=\"scrollRowThead\" 
	    strTable += "<tr class=\"rowstyle\" nowrap onclick=\"gpsmap.setGpsCarInfo('"+  carCode + "','"+ (i+1) +"',"+ coord +");\">";	
 ...
 * 获取当前选中GPS的数据
 */ 
InstantMap.prototype.getSelectedCarCode = function()
{
	var tableObj = document.getElementById( this.CarListTableID );	
 ...
 *
 */
InstantMap.prototype.showTraceWindow = function()
{
	//取得选中的车辆
 	var selectCarCode = gpsmap.getSelectedCarCode();
 	var regnumber = gpsmap.number;
 	var driver = gpsmap.person;		
 ...
 * 构建GPS车辆查询面板html字符串.
 */
InstantMap.prototype.outputHTML = function()
{
	  htmlTxt = '\
		...
		<tbody>\
			...
			<tr>\
				...
				<input type="button"   id="btnZDWatch" value="重点监控" onClick="setZdCarId();">\
						 &nbsp;&nbsp;\
					<input type="button"    value="历史轨迹" onClick="gpsmap.showTraceWindow();">\
					&nbsp;&nbsp;\
				</td> \
			</tr>\
		</tbody>\
