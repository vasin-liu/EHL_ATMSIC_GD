...
*@strAlign 位置定位，取值为left,right,和center
*/
function DivHolder(hTitle,hWidth,hHeight,hContent,closing,baseImageSrc,strAlign )
{
	...
	var tempStyle = "position:absolute;top:80px;";
	if(hWidth)
	{
	...
	var leftCssText = null ;
	if( "right" == strAlign )
	{
		var leftMargin = parseInt(docBody.rightMargin);
		leftCssText = docBody.clientWidth - (  hWidth  + ( isNaN(leftMargin) ? 0:leftMargin)); 
	}
	else if( "left" == strAlign)
	{
		leftCssText = docBody.leftMargin ;
	}else if( "center" == strAlign)
	{
		if( (docBody.clientWidth - hWidth) > 0)
		{
			leftCssText = (docBody.clientWidth - hWidth)/2;
		}
	}
	 
	if(leftCssText)
	{
		leftCssText = "left:" + leftCssText + "px;";
	}
	else
	{
		leftCssText = "left:100px;" ;
	}
 
	divHolder.style.cssText = tempStyle + leftCssText;
	...
	*/
	...
	{
		htmlTxt = '\
	    <table border="0" cellspacing="0" cellpadding="0" style="margin:0;table-layout:fixed;width:98%;height:98%">\
			<tbody>\
				<tr style="height:30px" id="h_headHtmlRow">\
    				<td width="6" align="left"  background="../images/holder/bubble_topleft.gif"  height="30" style="background-repeat:repeat-y;overflow:hidden;white-space: nowrap;" />&nbsp;</td>\
    				<td  height="30" nowrap  background="';
    				htmlTxt += baseImageSrc +  '/bubble_topcenter.gif" id="dragHead" width="' + centerWidth + '" ></td>\
    				<td width="25px" height="30" align="center"  background="';
		...
		htmlTxt += '</td>\
				...
				<tr>\
				    ...
				    <td colspan="2"   align="center" style="margin:0;background-color:#FFF;valign:center;vertical-align:middle;overflow:auto;text-align:center;height:' + centerHeight +'">\
				    	<div id="tdContainer"  style="margin:0;width:100%;overflow:scroll;background-color:#FFF;height:' + centerHeight +';">\
				    	</div>\
				    	</td>\
	...
	*/
	...
	this.resetHeight = function(nHeight)
	{
		if(! isNaN(parseInt(nHeight)))
		{
			var holderElement = DivHolder.getHolderElement();
			if(holderElement)
			{
				holderElement.style.height = nHeight;
				holderElement.parentNode.style.height = nHeight;
			}
			var objContent = document.getElementById("tdContainer");
			if( objContent )
			{
				objContent.style.height = nHeight - 34;
				objContent.parentNode.style.height = nHeight - 34;;
			}			
		}
	};
...
DivHolder.getHolderElement = function()
{
	var divId = 'div_holder';
	var holder = document.getElementById(divId);
	return holder;
...
BranchChoose.buildSubBranch = function(xmlHttp,layerCode ,htmlElement, imageBaseSrc)
{
	...
	for (var i = 0; i < rows.length ; i++) {
		...
		ele_span.onclick = new Function("BranchChoose.nodeClickHandler('" + ele_div.getAttribute("id") + "','" + results[0].text + "','"+results[1].text+ "','" + htmlElement+"','"+ imageBaseSrc+ "')");
	
		//添加ondblClick事件
		ele_span.ondblclick = new Function("BranchChoose.nodeDoubleClickHandler('" + ele_div.getAttribute("id") + "','" + results[0].text + "','"+results[1].text+ "','" + htmlElement+"')");
		
		//添加image子节点
		ele_span.appendChild(ele_image);
		
		//显示机构图片+名称	
		ele_span.innerHTML = BranchChoose.loadBlank(layerCode,ele_span.innerHTML + results[1].text);
