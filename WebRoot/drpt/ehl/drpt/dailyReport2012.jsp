...
<%
    ...
    String pname1 = StringHelper.obj2str(
		    session.getAttribute(Constants.PNAME_KEY), "");
...
<html>
	<head>
		...
		<%if ("0".equals(insrtOrUpdt) && !dra.chkTimeWithDept(tbdwid, tjrq)) {%>
			...
			window.close();
		<%}
	    if ("1".equals(insrtOrUpdt)
		    && !dra.chkTimeWithTjrq(tbdwid, bh, tjrq)) {%>
		...
		<script type="text/javascript" src="../../../general/js/jqgrid/jquery-1.7.1.min.js"></script>
		...
		<script type="text/javascript" src="${contextPath}base/js/new/place.js"></script>
		...
		<script type="text/javascript" src="../../js/drpt/dailyReport2012.js"></script>
...
.input11 {				border:0;
	...
	<body onLoad="doQuery('<%=insrtOrUpdt%>','<%=bh%>');">
		...
		<input type="hidden" id="isgsdd" value="${isgsdd}">
		<fieldset style="width:100%;height:100%;border:1px solid #CCCCCC" align="center">
            ...
            <table width="99%" align="center" class="tableInput" id="dataTable">
                <tr style="display:none">
                    ...
                    <td colspan="7">
                        ...
                        <input id="insrtOrUpdt" value="<%=insrtOrUpdt%>" type="hidden"  />                    </td>
                ...
                <tr>
                	<td class="td_b" colspan="8">
                		...
                		<div style="float:right;">
                			统计日期：<input maxLength="2" readOnly id="tjy" value="<%=tjy < 10 ? ("0" + Integer.toString(tjy)) : Integer
		    .toString(tjy)%>" class="input4" type="text" />月
                					<input maxLength="2" readOnly id="tjr" value="<%=tjr < 10 ? ("0" + Integer.toString(tjr)) : Integer
		    .toString(tjr)%>" class="input4" type="text" />日                		</div>                	</td>
                </tr>
                <tr>
			...
			<tr>
				<td class="td_r_b">26</td>
				<td colspan="7" class="td_b" align="left" style="text-align: left;vertical-align: middle;">
					<div id="flowTitle" style="width: 100%; text-align: center;display: none;"><div id='flowUtil' style='float:right;cursor: hand;text-decoration: underline;display: none;'>添加新流量</div></div>
					<table border="0" cellspacing="0" cellpadding="0" style="width: 100%;">
						<tr>
							<td style="width:252px;border-right: 1px solid #b5d6e6;text-align: left;vertical-align:middle;">
							辖区日车流量最大的路段
							<span id="btnFlowList" style="white-space: nowrap;display: none;text-decoration: underline;cursor:hand;margin:10 5 0 5;width:100px;">
								查看大队填报流量
							</span>
							</td>
							<td style="padding-bottom: 10px;">
								<table id="flow" border="0" cellspacing="0" cellpadding="0" style="display: inline;width: 490;text-align: center;">
									<tr>
										<th>道路等级</th>
										<th>道路名称</th>
										<th>路段</th>
...
<!-- 										<th>环比增减</th> -->
			...
			<tr>
				<td colspan="8" class="td_b">
					<div>
						<span style="width: 30%"> 填表人：<input id="TBR" <%=istr%>
							class="input7" type="text" />
						</span> <span style="width: 30%"> 审核人：<input id="SHR" <%=istr%>
							class="input7" type="text" />
						</span> <span style="width: 35%"> 联系电话：<input id="LXDH" <%=istr%>
							class="input7" type="text" />
						</span>
					</div>
				</td>
                	...
                	<td colspan="8" align="center">
                		<table <%="2".equals(insrtOrUpdt) ? "width=20%" : "width=60%"%>>
                			<tr>
                				<td width="35%" <%=istyle%>>
			                		<span class="search">
			                			<a href="#modify" name="modify" onclick="doModify();event.returnValue=false;"
											class="currentLocation"> <span class="lbl">提交</span>										</a>			                		</span>			                	</td>
			                	<!-- <td width="35%" <%=istyle%>>
			                		<span class="search">
			                			<a href="#reset" name="reset" onclick="doQuery('<%=insrtOrUpdt%>','<%=bh%>');event.returnValue=false;"
											class="currentLocation"> <span class="lbl">重置</span>										</a>			                		</span>		
												                	</td> -->
			                	<td <%="2".equals(insrtOrUpdt) ? "cospan=3" : "width=30%"%>>
			                		<span class="search">
			                			<a href="#" onclick="window.close()"
											class="currentLocation"> <span class="lbl">取消</span>										</a>			                		</span>                				</td>
                			</tr>
                		</table>                	</td>
                </tr>
            </table>
...
//暂停自动提交 	setInterval(savePm,20000)
</script>
<%
    }
%>
