...
//                var pcx_widht=(this._fake?(gridWidth-this._fake.entBox.offsetWidth):gridWidth);
                    ...
                    var wcor=this.entBox.clientWidth?(this.entBox.offsetWidth-this.entBox.clientWidth):0;
...
/*                         if ((_isMacOS)&&(_isFF))
				...
				if (this._botMb) {
					...
					this._botMb.style.width=(gridWidth+20)+"px";					
                     ...
                     //nb 072006:
                     aRow.style.height = (((gridHeight - zheight-1)<0 && _isIE)?20:(gridHeight - zheight-(this._isIE7s?2:1)))-(this.ftr?this.ftr.offsetHeight:0)+"px";
...
/*
   ...
   */
   ...
   this.setColOrder = function(sortStr){
      this.fldSort = sortStr.split(this.delim)
                          ...
                          if(headCol.length>0){
                             ...
                             var f_arr=[];
                                      for (var i=0; i<headCol.length; i++){
                                          a_list+=headCol[i].getAttribute("width")+this.delim;
                                          b_list+=headCol[i].getAttribute("type")+this.delim;
                                          c_list+=(headCol[i].getAttribute("align")||"left")+this.delim;
                                          d_list+=headCol[i].getAttribute("sort")+this.delim;
                                          e_list+=(headCol[i].getAttribute("color")!=null?headCol[i].getAttribute("color"):"")+this.delim;
                                          f_list+=(headCol[i].firstChild?headCol[i].firstChild.data:"").replace(/^\s*((.|\n)*.+)\s*$/gi,"$1")+this.delim;
                                f_arr[i]=headCol[i].getAttribute("format");
                                g_list+=headCol[i].getAttribute("id")+this.delim;
                                      }
...
*/
         ...
         if (this._c_order) strAr=this._swapColumns(strAr);
            for(var j=0;j<cellsCol.length;j++){
                 ...
                 if (css1) r.childNodes[j].style.cssText+=";"+css1;
                 css1=cellsCol[j].getAttribute("title");
                 if (css1) r.childNodes[j].title=css1;
            }
