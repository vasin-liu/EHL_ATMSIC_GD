	...
	this.check = function () {
		if ((dhtmlObject) && (dhtmlObject.onloadAction != null)) {
			if ((!dhtmlObject.xmlDoc.readyState) || (dhtmlObject.xmlDoc.readyState == 4)) {
				var jgid = parent.document.getElementById("jgid");
				if(jgid && jgid.value && jgid.value.substring(2,4)=="00"){
					sortZdAlarmTreeNode(dhtmlObject);
				}
...
dtmlXMLLoaderObject.prototype.getXMLTopNode = function (tagName) {
	...
	if ((_isIE) && (!this._retry)) {
		...
		alert(this.xmlDoc.responseText)
		this._retry = true;
...
dtmlXMLLoaderObject.prototype.loadXML = function (filePath, postMode, postVars, rpc) {
	...
	this.xmlDoc.setRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=UTF-8");
	this.xmlDoc.setRequestHeader("Accept-Charset", "UTF-8");
