...
function saveAsExcel(title,fileName,gridObj){
	...
	var url = "common.saveAsExcel.toExcel.d?title=" + title + "&fileName=" + fileName + "&tabHeader=" + tabHeader + "&tabData=" + tabData;
	url = encodeURI(url);
...
function saveFieldsAsExcel(title,fileName,header,dataSql){
	...
	var url = "common.saveAsExcel.fieldsToExcel.d?title=" + title + "&fileName=" + fileName + "&tabHeader=" + header + "&dataSql=" + dataSql;
	url = encodeURI(url);
 	var left = (screen.availWidth-800)/2;
    var top = (screen.availHeight-500)/2;
