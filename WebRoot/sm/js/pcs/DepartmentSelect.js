...
var G_JGMC = "";
var nodeID = "";
var $DEPT_BASE_PATH = ((window.baseInfo||{}).contextPath||"/EHL_ATMSIC_GD/")+"sm/";
 ...
 * @作者：郭亮亮
 * @函数说明：生成悬浮div方法
 * @参数：divID：层次编码, jgID：机构id, jgmc：机构名称, nodeId：机构输入框id,
 *                 state：设置根据鼠标位置定位标志,ignorenull：是否过滤掉机构中commandorder为空和‘0’的记录.
 * @创建日期：2008-08-12
 */
function showDepartTree(divID, jgID, jgmc, nodeId, top, left, ignorenull) {
	...
	var el = $(nodeId);
	var l = getPosition(el);
	// 给弹出窗口左右边距默认值0
	if (!top) {
		top = l.y + el.offsetHeight;
	}
	if (!left) {
		left = l.x;
	}
	...
	var html = "<span id=\""
			+ jgID
			+ "\" style=\"color:black;cursor:hand\" ondblclick=\"saveSelect();\" onclick=\"doOnClick('"
			+ divID + "','" + jgID + "','" + jgmc + "')\"><img id=\"" + "i"
			+ jgID + "\" src=\"" + $DEPT_BASE_PATH
			+ "image/tree/plusOpen.gif\"/>" + jgmc + "</span><br><div id=\""
			+ divID + "\" style=\"display:none\"></div>";

	Popup.prototype.showPopup('deptDiv', '请选择单位...', html);
	doOnClick(divID, jgID, jgmc, ignorenull);
 ...
 * 
 */
function doOnClick(divID, jgID, jgmc, ignorenull) {
	...
	if (img.src.indexOf("blankLeaf.gif") > 0) {
		...
		if (document.all(divID).style.display == "") {
			...
			img.src = $DEPT_BASE_PATH + "image/tree/plusOpen.gif";

		} else {
			document.all(divID).style.display = "";
			img.src = $DEPT_BASE_PATH + "image/tree/minusClosed.gif";
		}
	}
}
...
function writeElement(xmlHttp) {
	...
	for ( var i = 0; i < rows.length; i++) {
		...
		if (results[2].text > 0) {
			ele_image.setAttribute("src", $DEPT_BASE_PATH
					+ "image/tree/plusOpen.gif");

			// 为图片添加id，用于之后删除
			ele_image.setAttribute("id", "i" + results[0].text);
		} else {
			ele_image.setAttribute("src", $DEPT_BASE_PATH
					+ "image/tree/blankLeaf.gif");
		...
		ele_span.onclick = new Function("doOnClick('"
				+ ele_div.getAttribute("id") + "','" + results[0].text + "','"
				+ results[1].text + "')");
		...
		// 显示机构图片+名称
		ele_span.innerHTML = loadBlank(G_divID, ele_span.innerHTML
				+ results[1].text);
