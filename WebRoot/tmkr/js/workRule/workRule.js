...
function openDetail(index,page){
	if (index >= 0 && index < queryDatas.length) {
		var url = "workRule.jsp?page="+(page||2)+"&title=";
...
function setData(){
	...
	new Ajax.Request(url,{
		...
		onComplete:function(xmlHttp){
			...
			var plans = xmlDoc.getElementsByTagName("workRule");
...
function modify(isValid){
	if(isValid == true || modifyCheck()){
		if(!isValid){
			...
			var url = "tmkr.workRule.modify.d";
			new Ajax.Request(url,{
				method:"post",
				parameters : pageInfo,
				onComplete:function(xmlHttp){
