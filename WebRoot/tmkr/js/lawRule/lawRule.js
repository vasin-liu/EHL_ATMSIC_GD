...
function setTestValue(){
	$("pname").value = "李继忠";
	$("title").value = "新建法律法规";
	$("etime").value = new Date().format_(3);
}
/**设置添加页面值*/
...
function addCheck(){
	...
	var etime = $("etime");
	var evalue = etime.value;
	if(evalue == ""){
		alert("请选择法规执行日期！");
		etime.focus();
		return false;
	}else{
		pageInfo.etime = evalue;
	}
...
/**查询页面对象字段名*/
var fnames = [ "id", "jgid", "jgmc", "pname", "time", "title", "etime","apath" ];
/**查询页面对象字段描述*/
var fdescs = ["编号","机构编号","机构名称","录入人","录入时间","标题","法规执行日期","附件路径"];
/**查询列表数据*/
var queryDatas;
/**查询单条数据*/
var queryData;
...
function query(){
	...
	var sql = "select lr.id,lr.jgid," +
	"(select jgmc from t_sys_department where jgid=lr.jgid)," +
	"lr.pname,to_char(lr.time,'yyyy-mm-dd hh24:mi'),lr.title," +
	"to_char(lr.etime,'yyyy-mm-dd'),lr.apath";
	sql += " from t_tmkr_lawRule lr";
	sql += " where 1=1";
	/*
	//自己的预案或者其他单位让自己查看的预案
	sql += " and (lr.jgid='"+baseInfo.jgid+"' or (instr(lr.ajgid,'"+baseInfo.jgid+"')!=0))";
	//筛选预案
	sql += "and "+Dept.siftChilds(window.G_jgID,"lr.jgid");
	*/
	//起始时间
	var start = pageInfo.stime;
	sql += siftTime(start+" 00:00:00","lr.time","<=");
	//结束时间
	var end = pageInfo.etime;
	sql += siftTime(end+" 23:59:59","lr.time",">=");
	//标题
	var title = pageInfo.title;
	sql += " and lr.title like '%"+title+"%'";	
	...
	if (!queryDatas || queryDatas.length <= 0) {
		...
		for ( var i = 0; i < queryDatas.length; i++) {
			...
			if(queryData.jgid == baseInfo.jgid){
				...
				odata.push(["btndelete1.gif","删除","deleteInfo("+i+")"]);
			}else{
				odata.push(["lock.png","处理",""]);
				odata.push(["btndelete2.gif","删除",""]);
			}
...
function openDetail(index,page){
	if (index >= 0 && index < queryDatas.length) {
		var url = "lawRule.jsp?page="+(page||2);
...
function setData(){
	...
	new Ajax.Request(url,{
		...
		onComplete:function(xmlHttp){
			...
			var plans = xmlDoc.getElementsByTagName("lawRule");
...
function deleteInfo(index){
	var isSure = confirm("确认要删除信息吗？");
	if(isSure){
		var id = queryDatas[index].id;
		var url = "tmkr.lawRule.delete.d";
		new Ajax.Request(url,{
			method:"post",
			parameters:{id:id},
			onComplete:function(xmlHttp){
